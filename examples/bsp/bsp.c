...
static uint32_t bsp_led_indication(bsp_indication_t indicate)
{
    ...
    switch (indicate)
    {
        ...
        case BSP_INDICATE_USER_STATE_0:
            LEDS_OFF(LEDS_MASK & ~(BSP_LED_1_MASK | BSP_LED_0_MASK));
            LEDS_ON(BSP_LED_1_MASK);
            m_stable_state = indicate;
            break;

        case BSP_INDICATE_USER_STATE_1:
            LEDS_OFF(LEDS_MASK & ~(BSP_LED_2_MASK | BSP_LED_0_MASK));
            LEDS_ON(BSP_LED_2_MASK);
            m_stable_state = indicate;
            break;

        case BSP_INDICATE_USER_STATE_2:
            //LEDS_OFF(LEDS_MASK & ~(BSP_LED_0_MASK | BSP_LED_1_MASK | BSP_LED_2_MASK));
...
uint32_t bsp_init(uint32_t type, uint32_t ticks_per_100ms, bsp_event_callback_t callback)
{
    ...
    if (type & BSP_INIT_BUTTONS)
    {
        ...
        {
            err_code = bsp_event_to_button_action_assign(num, BSP_BUTTON_ACTION_PUSH, BSP_EVENT_NOTHING);
        }
