...
#define DEVICE_NAME                      "Kz_HRM"                               /**< Name of device. Will be included in the advertising data. */
...
static ble_uuid_t m_sr_uuids[] =  {{LBS_UUID_SERVICE, 					 BLE_UUID_TYPE_BLE}
									};
#ifdef BLE_DFU_APP_SUPPORT    
...
/* led写事件处理 */
static void on_led_char_write(ble_lbs_t * p_lbs, uint8_t new_state)
{
	if(1 == new_state)
	{
		bsp_indication_text_set(BSP_INDICATE_USER_STATE_0,NULL);
	}
	else if(2 == new_state)
	{
		bsp_indication_text_set(BSP_INDICATE_USER_STATE_1,NULL);
	}
	else
	{
		bsp_indication_text_set(BSP_INDICATE_USER_STATE_OFF,NULL);
	}
}
...
static void services_init(void)
{
	...
	// Initialize led and button Service.
    memset(&lbs_init, 0, sizeof(lbs_init));

    lbs_init.led_write_handler = on_led_char_write;
...
void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
		...
		case BSP_EVENT_KEY_0:
			err_code = ble_lbs_on_button_change(&m_lbs,0x01);
			if (err_code != NRF_ERROR_INVALID_STATE)
            {
                APP_ERROR_CHECK(err_code);
            }
		...
		case BSP_EVENT_KEY_1:
			err_code = ble_lbs_on_button_change(&m_lbs,0x02);
			if (err_code != NRF_ERROR_INVALID_STATE)
            {
                APP_ERROR_CHECK(err_code);
            }
